...
class SignalDg:
	...
	def __dataConverter(self, lista, numero, value):
		...
		for x in lista:
			...
			ax_int = np.linspace(testx[0],testx[-1], 81)
			ay_int = f(ax_int)
			...
			print("b len. ", len(testx), "  c len. ", len (cx_int))
	...
	def __pikeSearcher(self, sDG, x, y, aleluya):
		...
		while y > 0:
			if control:
				if self.__lineDefiner(trace, y , parameter[1], parameter[0], media, "i"):
					#print("\n original y = ", y)
					control = False
					d = y
					y = d
				...
				while True:
					if y+z  == d :
						break
					if self.__lineDefiner(trace, y + z , parameter[1], parameter[0], media, "d"):
						u =  y + z
						break
					z = z + 5
				#print(" con  u = ", u, "y d =", d)
		...
		'''while y < len(trace):
			if control:
				if self.__lineDefiner(trace, y , parameter[1], parameter[0], media, "d"):
					control = False
					d = y
					y = d
			elif trace[y] >= self.__minimumPointEvent:
				...
				while True:
					if y-z  == d :
						break
					if self.__lineDefiner(trace, y - z , parameter[1], parameter[0], media, "i"):
						u =  y - z
						break
					z = z + 5
				if u > d:
	...
	def __lineDefiner(self, trace, punto,cantidad, variacion, media, dir):
		resultado = True
		counter = 0
		print("\n punto +cant ", punto +cantidad, "punto -cantidad", punto -cantidad)
		print("testeo", punto,"der",abs(self.__localMean(trace, punto, punto + cantidad)), "izq",   abs(self.__localMean(trace, punto - cantidad, punto )) , " media", media)

		if (abs(self.__localMean(trace, punto, punto + cantidad)) - variacion) >  media and (abs(self.__localMean(trace, punto - cantidad, punto )) - media) >  media:
				resultado = False
